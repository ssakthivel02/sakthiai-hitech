...
function isPortAvailable(port: number): Promise<boolean> { return new Promise(resolve => { const server = net.createServer(); server.listen(port, "0.0.0.0", () => server.close(() => resolve(true))); server.on("error", () => resolve(false)); }); }
...
async function startServer() {
  ...
  app.get("/healthz", (_req, res) => res.status(200).json({ status: "alive", service: "sakthiai", environment: process.env.NODE_ENV || "unknown" }));
  app.get("/readyz", async (_req, res) => {
    const db = await getDb();
    const databaseReady = Boolean(db);
    const authReady = configured(ENV.cookieSecret, ENV.oidcAuthorizationUrl, ENV.oidcTokenUrl, ENV.oidcUserInfoUrl, ENV.oidcClientId);
    const llmReady = configured(ENV.llmApiUrl, ENV.llmModel);
    const storageReady = configured(ENV.storageBucket, ENV.storageAccessKeyId, ENV.storageSecretAccessKey);
  ...
  const preferredPort = parseInt(process.env.PORT || "3000"); const port = await findAvailablePort(preferredPort); if (port !== preferredPort) console.log(`Port ${preferredPort} is busy, using port ${port} instead`); server.listen(port, "0.0.0.0", () => console.log(`Server running on http://0.0.0.0:${port}/`));
}
