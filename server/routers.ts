...
async function requireOwnedConversation(userId: number, workspaceId: number, conversationId: number) {
  const db = await getDb();
  if (!db) throw new TRPCError({ code: "INTERNAL_SERVER_ERROR", message: "Database unavailable" });
  const owned = await db.select({ id: conversations.id }).from(conversations).where(and(eq(conversations.id, conversationId), eq(conversations.workspaceId, workspaceId), eq(conversations.userId, userId))).limit(1);
...
export const appRouter = router({
  ...
  files: router({
    ...
    upload: protectedProcedure.input(workspaceInput.extend({ projectId: z.number().int().positive().optional(), filename: z.string().min(1).max(255), mimeType: z.string().min(1), dataBase64: z.string().min(1) })).mutation(async ({ ctx, input }) => {
      ...
      const created = await db.select().from(documents).where(and(eq(documents.workspaceId, input.workspaceId), eq(documents.contentHash, contentHash))).orderBy(desc(documents.id)).limit(1); const document = created[0]; if (!document) throw new TRPCError({ code: "INTERNAL_SERVER_ERROR", message: "Document persistence failed" });
  ...
  chat: router({
    history: protectedProcedure.input(workspaceInput.extend({ conversationId: z.number().int().positive() })).query(async ({ ctx, input }) => { await requireWorkspace(ctx.user.id, input.workspaceId); await requireOwnedConversation(ctx.user.id, input.workspaceId, input.conversationId); return getConversationMessages(input.workspaceId, input.conversationId); }),
    send: protectedProcedure.input(workspaceInput.extend({ conversationId: z.number().int().positive().optional(), message: z.string().trim().min(1).max(12000), language: z.enum(["en", "ta"]).default("en") })).mutation(async ({ ctx, input }) => {
      ...
      if (conversationId) await requireOwnedConversation(ctx.user.id, input.workspaceId, conversationId);
